...
function normalizeTruckPayload(payload) {
  ...
  return {
    ...
    insuranceProvider: normalized.insuranceProvider || firstNonEmpty(flat['Insurance Provider'], flat['Insurer'], flat['Insurance Company'], flat['Company Name'], flat.insurer, flat.insuranceProvider),
    policyNo: normalized.policyNo || firstNonEmpty(flat['Policy No'], flat['Policy Number'], flat['Policy No.'], flat.policyNo, flat.policy_number),
    insuranceExpiry: parseDateToText(firstNonEmpty(flat['Insurance Expiry'], flat['Expiry Date'], flat['Policy Expiry'], flat.insExpiry, flat.expiry, flat.expiryDate)),
    coverageType: firstNonEmpty(flat['Coverage Type'], flat['Insurance Type'], flat.coverageType),
...
function mergeTruckPayloads(documentType, payloads) {
  ...
  return {
    ...
    bodyType: documentType === 'rc' ? firstNonEmpty(...normalized.map((p) => p.bodyType)) : null,
    capacity: documentType === 'rc' ? firstNonEmpty(...normalized.map((p) => p.capacity)) : null,
    purchaseDate: documentType === 'purchase_invoice' ? firstNonEmpty(...normalized.map((p) => p.purchaseDate)) : null,
    purchasePrice: documentType === 'purchase_invoice' ? firstNonEmpty(...normalized.map((p) => p.purchasePrice)) : null,
    ...
    coverageType: firstNonEmpty(...normalized.map((p) => p.coverageType)),
